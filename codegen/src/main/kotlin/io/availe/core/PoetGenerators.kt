...
fun generateValueClass(wrapper: InlineWrapper): TypeSpec = TypeSpec
    .classBuilder(wrapper.name)
...
fun generateEnum(spec: EnumSpec): TypeSpec {
    val b = TypeSpec.enumBuilder(spec.name)
    spec.values.forEach { b.addEnumConstant(it.uppercase()) }
    return b.build()
}
